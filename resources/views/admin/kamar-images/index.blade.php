...
@section('title', 'Kelola Kamar Images')
...
@section('breadcrumb')
    ...
    <li class="inline-flex items-center">
        <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
        <span class="text-gray-500">Kelola Kamar Images</span>
    </li>
@endsection
...
<div class="bg-white/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20">
    ...
    <div class="p-6 border-b border-gray-200/50 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center space-x-3">
            ...
            <h3 class="text-xl font-bold text-gray-900">Daftar Kamar Images</h3>
        </div>
        <a href="{{ route('admin.kamar-images.create') }}" 
    ...
    <div class="p-6 border-b border-gray-200/50">
        <div class="relative max-w-md">
            ...
            <input type="text" id="searchKamarImage" 
                   ...
                   placeholder="Cari kamar image...">
        </div>
    </div>
    
    <!-- Table -->
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200" id="kamarImageTable">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors duration-200">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $kamarImage->created_at->format('d M Y') }}</div>
                        <div class="text-sm text-gray-500">{{ $kamarImage->created_at->format('H:i') }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex items-center space-x-2">
                            <a href="{{ route('admin.kamar-images.show', $kamarImage->image_id) }}" 
                            ...
                            <a href="{{ route('admin.kamar-images.edit', $kamarImage->image_id) }}" 
                            ...
                            <form action="{{ route('admin.kamar-images.destroy', $kamarImage->image_id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
                                        ...
                                        title="Delete"
                                        onclick="return confirm('Apakah Anda yakin ingin menghapus kamar image ini?')">
    ...
    <!-- Pagination -->
    @if($kamarImages->hasPages())
    <div class="px-6 py-4 border-t border-gray-200/50 flex items-center justify-between">
        <div class="text-sm text-gray-700">
            Showing {{ $kamarImages->firstItem() }} to {{ $kamarImages->lastItem() }} of {{ $kamarImages->total() }} results
        </div>
        <div class="flex items-center space-x-2">
            {{ $kamarImages->links('pagination::tailwind') }}
        </div>
    </div>
    @endif
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const searchInput = document.getElementById('searchKamarImage');
    const tableRows = document.querySelectorAll('#kamarImageTable tbody tr');
