...
@section('title', 'Detail Kamar Image')
...
@section('breadcrumb')
    ...
    <li class="inline-flex items-center">
        <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
        <span class="text-gray-500">Detail Kamar Image</span>
    </li>
@endsection
...
<div class="bg-white/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20">
    ...
    <div class="p-6 border-b border-gray-200/50 flex items-center space-x-3">
        ...
        <h3 class="text-xl font-bold text-gray-900">Detail Kamar Image</h3>
    </div>
    ...
    <div class="p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-xl p-4 flex items-center justify-center h-96">
                    <img src="{{ $kamarImage->image_url }}" alt="Kamar Image" class="max-h-full max-w-full object-contain">
                </div>
            ...
            <div>
                <div class="mb-6">
                    ...
                    <div class="text-lg font-semibold text-gray-900">{{ ucfirst($kamarImage->kamar->detailKamar->tipe_kamar) }} Room</div>
                    <div class="text-gray-600">{{ $kamarImage->kamar->hotel->nama_hotel }}</div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="text-gray-900 break-words">
                        @if(filter_var($kamarImage->image_url, FILTER_VALIDATE_URL))
                            ...
                            <div class="mt-2">{{ $kamarImage->image_url }}</div>
                        @else
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                <i class="fas fa-file-image mr-1"></i> File
                            </span>
                            <div class="mt-2">{{ basename($kamarImage->image_url) }}</div>
                        @endif
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="text-gray-900">{{ $kamarImage->created_at->format('d M Y, H:i') }}</div>
                </div>
                
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Updated At</label>
                    <div class="text-gray-900">{{ $kamarImage->updated_at->format('d M Y, H:i') }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-4 mt-8">
            ...
            <form action="{{ route('admin.kamar-images.destroy', $kamarImage->image_id) }}" method="POST" class="inline">
                ...
                @method('DELETE')
                <button type="submit" 
                        class="inline-flex items-center px-4 py-2 bg-red-500 hover:bg-red-60 text-white font-medium rounded-xl shadow-lg hover:shadow-xl transition-all duration-300"
